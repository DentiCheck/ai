...
class LlmClient:
    ...
    def simple_chat(self, user_message: str, system_prompt: str = prompts.SYSTEM_PERSONA_DOCTOR) -> str:
        """
        간단한 챗봇 대화를 수행합니다 (일괄 응답).

        Args:
            user_message (str): 사용자가 입력한 질문이나 메시지
            system_prompt (str): AI의 페르소나나 역할을 정의하는 프롬프트
    ...
    def stream_chat(self, user_message: str, system_prompt: str = prompts.SYSTEM_PERSONA_DOCTOR):
        """
        실시간 스트리밍으로 답변을 생성합니다.

        Args:
            user_message (str): 사용자가 입력한 메시지
            system_prompt (str): AI 페르소나

        Returns:
            generator: 텍스트 조각(chunk)을 반환하는 제너레이터
        """
        messages = [
            SystemMessage(content=system_prompt),
            HumanMessage(content=user_message)
    ...
    def generate_report(self, risk_level: str, detections: str, actions: str) -> str:
        """
        AI 분석 결과를 바탕으로 전문 소견 리포트를 생성합니다.

        Args:
            risk_level (str): 위험도 레벨 (정상/주의/위험)
        ...
        Returns:
            str: 완성된 리포트 내용
        """
        prompt_text = prompts.REPORT_GENERATION_TEMPLATE.format(
            risk_level=risk_level,
            detections=detections,
            actions=actions
        )
        return self.simple_chat(prompt_text)
