...
본 시스템은 Milvus의 L2 거리 점수를 기반으로 다음과 같이 신뢰도를 계산합니다:
- **Case A (거리가 1 미만일 때)**: $100 - (Distance \times 100)$
- **Case B (거리가 1 이상일 때)**: $100 / (1 + Distance)$
- **의미**: 거리가 0에 가까울수록(완벽 일치) 100%에 수렴하며, 거리가 멀어질수록 신뢰도가 급격히 낮아집니다.
